...
public class UserServiceImpl implements  UserService{
    ...
    @Override
    public int queryUserByname(User user, String attribute, String checkcode) {
        String key = "";
        User reuser =null;
        if (attribute.equals(checkcode)) {
            reuser = userMapper.queryUserByname(user);
        }
        if (reuser == null) {//用户名不存在
            return 3;
        }else{
            key = "login" + reuser.getUid();
            if (user.getUpwd() != null && !"".equals(user.getUpwd()) && user.getUpwd().equals(reuser.getUpwd())) {
                redisTemplate.opsForValue().set(key, user.getUid()+"");
            }else{
                return 4;
            }
        }
